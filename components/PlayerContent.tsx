...
const PlayerContent: FC<PlayerContentProps> = (props) => {
  ...
  const onPlayNext = () => {
    ...
    const currentSongId = player.ids.findIndex((id) => id === player.activeId);
    const nextSong = player.ids[currentSongId + 1];
  ...
  const onPlayPrevious = () => {
    ...
    const currentSongId = player.ids.findIndex((id) => id === player.activeId);
    const previousSong = player.ids[currentSongId - 1];
    ...
    <div className={"grid grid-cols-2 md:grid-cols-3 h-full"}>
      ...
      >
        ...
        <div
          onClick={handlePlay}
          className={
            "flex items-center justify-center h-10 rounded-full bg-white p-1 cursor-pointer"
          }
        >
          <Icon size={30} className={"text-black"} />
        </div>
